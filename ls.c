...
static const umode_t MBIT[] = {S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP, S_IWGRP,
                               S_IXGRP, S_IROTH, S_IWOTH, S_IXOTH};

/* The special bits. If set, display SMODE0/1 instead of MODE0/1 */

static const umode_t SBIT[] = {0, 0, S_ISUID, 0, 0, S_ISGID, 0, 0, S_ISVTX};
...
static void list_single(const char *name, struct stat *info) {
    ...
    if (display_fmt == FMT_LONG) {
        ...
        for (i = 0; i < 9; i++) {
            if (mode & SBIT[i]) {
                scratch[i + 1] = (mode & MBIT[i]) ? SMODE1[i] : SMODE0[i];
            } else {
                scratch[i + 1] = (mode & MBIT[i]) ? MODE1[i] : MODE0[i];
            }
        }
        newline();
        wr(scratch, 10);
